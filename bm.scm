...
(define (bm-search-full pattern pattern-reversed patlen position text)
  (if (null? text)
      '() (let ((char (list-get text (- patlen 1))))
	    (if (eq? (car pattern-reversed) char)
		(let ((mismatch-position (first-mismatch pattern-reversed (reverse (sublist text 0 patlen)))))
		  (if (= mismatch-position patlen) ; Match found
		      ...
		      (begin (set! char (list-get text (- patlen mismatch-position)))
			     (let ((char-occurrences (reverse (occurrences char pattern))))
				   ...
				   (bm-search-full pattern pattern-reversed patlen
						   (+ position (- patlen mismatch-position)) text)
				   (if (> (car char-occurrences) (- (- patlen mismatch-position) 1))
				       ...
				       (bm-search-full pattern pattern-reversed patlen
						       (+ position (- (- patlen (car char-occurrences)) 1))
						       (sublist-from text (- (- patlen (car char-occurrences)) 1)))))))))
		(let ((char-occurrences (reverse (occurrences char pattern))))
		  (if (null? char-occurrences)
		      (bm-search-full pattern pattern-reversed patlen (+ position patlen) (sublist-from text patlen))
		      (bm-search-full pattern pattern-reversed patlen
				      (+ position (- (- patlen (car char-occurrences)) 1))
				      (sublist-from text (- (- patlen (car char-occurrences)) 1)))))))))
